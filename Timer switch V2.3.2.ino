...
// 定义输出引脚
const int outputPinForGroup1 = 2;
const int outputPinForGroup2 = 0;
...
void loop() {
    ...
    // 如果设置了定时，判断是否在定时时间段内
    if (isTimed) {
        bool isInOnPeriodForGroup1 = false;
        bool isInOnPeriodForGroup2 = false;
        // 修改后的时间判断条件
        if ((currentTime.hour > startHourForGroup1 || (currentTime.hour == startHourForGroup1 && currentTime.minute >= startMinuteForGroup1)) &&
            (currentTime.hour < endHourForGroup1 || (currentTime.hour == endHourForGroup1 && currentTime.minute < endMinuteForGroup1))) {
            isInOnPeriodForGroup1 = true;
        }
        // 增加第二组定时的判断
        if ((currentTime.hour > startHourForGroup2 || (currentTime.hour == startHourForGroup2 && currentTime.minute >= startMinuteForGroup2)) &&
            (currentTime.hour < endHourForGroup2 || (currentTime.hour == endHourForGroup2 && currentTime.minute < endMinuteForGroup2))) {
            isInOnPeriodForGroup2 = true;
        }
        // 根据状态设置输出引脚
    ...
    if (isTimed) {
        ...
        // 倒数第四行固定显示提示信息
        display.setCursor(0, SCREEN_HEIGHT - 24);
        display.println("Timer switch V2.3.2");
    }
