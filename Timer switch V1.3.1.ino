...
void handleSetTime() {
    if (server.method() == HTTP_POST) {
        ...
        // 更新定时显示字符串
        timingDisplay = String(startHour) + ":" + String(startMinute) + " - " + String(endHour) + ":" + String(endMinute);
...
void loop() {
    ...
    // 检查是否过了一分钟，如果是则更新 IP 地址
    unsigned long currentTime = millis();
    if (currentTime - lastCheckTime >= 60000) {
        IPAddress newLocalIP = WiFi.localIP();
        String newIpAddressStr = String(newLocalIP[0]) + "." +
                                 String(newLocalIP[1]) + "." +
                                 String(newLocalIP[2]) + "." +
                                 String(newLocalIP[3]);
        if (newIpAddressStr!= ipAddressStr) {
            ipAddressStr = newIpAddressStr;
        }
        lastCheckTime = currentTime;
    }

    // 在 OLED 屏幕上显示日期、时间、开关状态、IP 地址和特定字符串
    display.clearDisplay();
    String dateTimeStr = String(currentYear) + "-" + String(currentMonth + 1) + "-" + String(currentDay) + " " + String(currentHour) + ":" + String(currentMinute) + ":" + String(currentSecond);
    String newIpAddressStr = ipAddressStr;
    if (ipAddressStr == "IP address:192.168.5.24") {
        newIpAddressStr = "IP:192.168.5.24";
    }
    display.setCursor(0, 0);
    display.println(dateTimeStr + "    " + newIpAddressStr);
    int pinState = digitalRead(outputPin);
    String switchStatus = (pinState == HIGH? "On" : "Off");
    if (isTimed) {
        display.println("Time: Timed - " + switchStatus);
    } else {
        display.println("Time: Not Timed - " + switchStatus);
    }
    display.setCursor(0, SCREEN_HEIGHT - 24);
    display.println("Timer switch V1.3.1");
    // 显示定时时间在倒数第五行
    if (isTimed) {
        display.setCursor(0, SCREEN_HEIGHT - 40);
        display.println(timingDisplay);
    } else {
        display.println();
    }
